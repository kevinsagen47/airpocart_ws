...
def P_angular_control(data,RoW):
   #global RoV,RoW
   global on, soft_start,turn_back_left,turn_back_right,RoV
   offset = -0.03
   middle = data[0]-0.01
   x = 0.7
   if (x>0.6):
      if (data[0]==-1  or  (middle<0.03 and middle>-0.03) or  data[2]>max_depth or on==0):
         RoW=0.0
         turn_back_left = False
         turn_back_right = False

      elif (middle<=-0.03 and turn_back_left == False) or (turn_back_right==True) and RoV>0:#too right
         ...
         if(RoW>=w_max):
            ...
            RoW=RoW+0.02
         '''
      elif (middle>=0.03 and turn_back_right==False) or (turn_back_left==True ) and RoV>0:#too left
         RoW = 0.2#turn left
         if(middle>0.2):
